...
public class UIParticleSystem : MonoBehaviour {
    ...
    public static UIParticleSystem OBJ_UIparticle;
    ...
    private void Init()
    {
        ...
        for (int i = 0; i < ParticlePool.Length; i++)
        {
            ...
            ParticlePool[i].GetComponent<Image>().preserveAspect = true;
        }
    }
    ...
    private IEnumerator CoParticleFly(Image particle)
    {
        ...
        //apply angle
        emissonAngle = Quaternion.AngleAxis(Random.Range(-EmissionAngle / 2f, EmissionAngle / 2f), Vector3.forward) * emissonAngle * Time.deltaTime;  //
        //normalize
        ...
        while (particleLifetime < Lifetime)
        {
            ...
            //apply gravity
            gravityForce = Vector3.up * Gravity * particleLifetime * Time.deltaTime;   //
