...
public class clickText : MonoBehaviour, IPointerClickHandler
{
    ...
    public void THI_highlightWordsAfterRetrieve(string colorcode)
    {
       ...
       /* for (int i = 0; i < STRA_wordsAfter.Length; i++)
        ...
        foreach (string s in STRA_wordsBefore)
        {
           // Debug.Log("Hightlight 3");
            for (int i = 0; i < Main_Blended.OBJ_main_blended.LS_WORDS.Count; i++)
            {
                if (s == Main_Blended.OBJ_main_blended.LS_WORDS[i])
                {
                    int index = Array.IndexOf(STRA_wordsBefore, s);
                    IL_IndexList.Add(index);
                    Debug.Log(index);
                }
               // Debug.Log("Hightlight 4");
            }
           // Debug.Log("Hightlight 5");
        }
